...
class CustomizeScene(Scene):
    ...
    def _cycle_part(self,part,direction):
        ...
        self.spark_burst.trigger(
            CHARACTER_CENTER_X,
            CHARACTER_BOTTOM_Y-CHARACTER_HEIGHT//2
        )
    ...
    def handle_input(self,event):
        ...
        if event.type==pygame.MOUSEBUTTONDOWN:
            ...
            for p,r in self.selector_rects.items():
                if r['right'].collidepoint(event.pos):
                    self._cycle_part(p,'next')
                elif r['left'].collidepoint(event.pos):
                    self._cycle_part(p,'prev')
    ...
    def draw(self,screen):
        ...
        screen.blit(
            self.font.render("INGRESA TU NOMBRE:",True,(255,255,255)),
